...
// Replace the assessmentActions function in frontend/app.js
function assessmentActions(item, variant = "card") {
  const a = normalizeAssessment(item, item && item.idx);
  if (!a || !a.hash) return "";
  const cls = variant === "row" ? "btn-icon" : "btn";
...
// Replace the renderDossierModal function in frontend/app.js
function renderDossierModal(item) {
  const consensus = item.consensus_raw || {};
  const meta = item.judge_metadata || consensus._judge_metadata || {};
  const warnings = item.warnings || [];
...
2. Backend Updates: api.pyReplace request_llm_review and arcade_finish in your backend entirely with the codes below.  Python# Replace the request_llm_review function in api.py
...
def request_llm_review(file_hash: str, payload: ReviewRequest, request: Request):
    ...
    verdict = "llm-concerns"
    comment = "Machine review attempted but no models were reachable."
    ...
    if judge_routes:
        for route in judge_routes:
            ...
            if not attempt.get("api_failed", True):
                ...
                verdict = f"llm-{attempt.get('verdict', 'sound').lower()}"
                comment = (
                    f"Machine review generated by {route['model']} via {route['provider']}.\n\n"
                    f"{attempt.get('opinion', '')}"
                )
                break
    ...
    add_log(f"Real LLM review recorded for {file_hash[:12]}… ({verdict})")
    return {"requested": True, "verdict": verdict, "charged": fee,
            "balance": get_piq_balance(identity["wallet"], identity["orcid"])["balance"],
            "message": "LLM review generated successfully and badge attached."}

# Replace the arcade_finish function in api.py
